// cmd/checkcard.go
// checkcard CLI 커맨드 — 체크카드 사용내역 조회
...
func RunCheckcard(cfg *config.Config, args []string) {
	...
	fs := flag.NewFlagSet("checkcard", flag.ExitOnError)
	accountID := fs.String("account-id", "", "체크카드 항목 ID (생략 시 전체)")
	...
	data, err := client.GetCheckcard(api.CardQuery{
